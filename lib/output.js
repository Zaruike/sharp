...
const errJxlSave = new Error('JXL output requires libvips with support for JPEG-XL');
 ...
 * Use these JP2 options for output image.
 *
 * Requires libvips compiled with support for JPEG-XL.
 * The prebuilt binaries do not include this - see
 * {@link https://sharp.pixelplumbing.com/install#custom-libvips installing a custom libvips}.
 *
 * @example
 * // Convert any input to lossless jxl output
 * const data = await sharp(input)
 *   .jxl({ lossless: true })
 *   .toBuffer();
 *
 * @example
 * // Convert any input to very high quality jxl output
 * const data = await sharp(input)
 *   .jxl({
 *     quality: 100,
 *     tier: 0,
 ...
 * @param {Object} [options] - output options
 * @param {number} [options.quality=80] - quality, integer 1-100
 * @param {number} [options.tier=0] - decode speed tier
 * @param {number} [options.distance=1.0] - maximum encoding error
 * @param {number} [options.effort=4] - encoding effort
...
function jxl(options) {
  if (!this.constructor.format.jxl.output.buffer) {
    throw errJxlSave;
  }

  if (is.object(options)) {
    if (is.object(options.effort)) {
      if (is.integer(options.effort) && is.inRange(options.effort, 1, 7)) {
        this.options.jxlEffort = options.effort;
      } else {
        throw is.invalidParameterError('effort', 'integer between 1 and 7', options.effort);
      }
    }

    if (is.object(options.quality)) {
      if (is.integer(options.quality) && is.inRange(options.quality, 1, 100)) {
        this.options.jxlQuality = options.quality;
      } else {
        throw is.invalidParameterError('quality', 'integer between 1 and 100', options.quality);
      }
    }

    if (is.object(options.tier)) {
      if (is.integer(options.tier) && is.inRange(options.tier, 0, 4)) {
        this.options.jxlTier = options.tier
      } else {
        throw is.invalidParameterError('tier', 'integer between 0 and 4', options.tier);
      }
    }
    if (is.defined(options.lossless)) {
      if (is.bool(options.lossless)) {
    ...
    if (is.defined(options.distance)) {
      if (is.number(options.distance) && options.distance >= 0 && options.distance <= 15) {
        this.options.jxlDistance = options.distance;
      } else {
        throw is.invalidParameterError('distance', 'number greater than zero and less than fifteen', options.distance);
      }
    }
  }
